...
@AllArgsConstructor
@Entity
@Table(name = "reportes_diarios", indexes = {
...
public class ReporteDiario {
        ...
        @JoinColumn(name = "tarea_id", nullable = false, foreignKey = @ForeignKey(name = "fk_reporte_tarea"))
        private TareaAsignada tarea;

        @NotNull(message = "El empleado es obligatorio")
        @ManyToOne(fetch = FetchType.LAZY, optional = false)
        @JoinColumn(name = "empleado_id", nullable = false, foreignKey = @ForeignKey(name = "fk_reporte_empleado"))
        ...
        @Column(name = "porcentaje_avance", precision = 5, scale = 2, columnDefinition = "DECIMAL(5,2) DEFAULT 0.00")
        ...
        @Column(name = "fecha_reporte", nullable = false, insertable = false, updatable = false, columnDefinition = "TIMESTAMP DEFAULT CURRENT_TIMESTAMP")

...
public class BoletaPago {
        ...
        @JoinColumn(name = "empleado_id", nullable = false, foreignKey = @ForeignKey(name = "fk_boleta_empleado"))
        private Empleado empleado;

        @NotNull(message = "La planilla es obligatoria")
        @ManyToOne(fetch = FetchType.LAZY, optional = false)
        @JoinColumn(name = "planilla_id", nullable = false, foreignKey = @ForeignKey(name = "fk_boleta_planilla"))
        ...
        @Column(name = "dias_trabajados", nullable = false, columnDefinition = "INT DEFAULT 0")
        private Integer diasTrabajados = 0;

        @Min(value = 0, message = "Los días no trabajados no pueden ser negativos")
        @Column(name = "dias_no_trabajados", nullable = false, columnDefinition = "INT DEFAULT 0")
        private Integer diasNoTrabajados = 0;

        @DecimalMin(value = "0.00")
        @Column(name = "asignacion_familiar", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal asignacionFamiliar = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "bono_beta", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal bonoBeta = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "horas_extra_pago", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal horasExtraPago = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "otros_bonos", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal otrosBonos = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "descuento_inasistencia", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal descuentoInasistencia = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "otros_descuentos", precision = 10, scale = 2, columnDefinition = "DECIMAL(10,2) DEFAULT 0.00")
        private BigDecimal otrosDescuentos = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "total_ingresos", precision = 12, scale = 2, columnDefinition = "DECIMAL(12,2) DEFAULT 0.00")
        private BigDecimal totalIngresos = BigDecimal.ZERO;

        @DecimalMin(value = "0.00")
        @Column(name = "total_descuentos", precision = 12, scale = 2, columnDefinition = "DECIMAL(12,2) DEFAULT 0.00")
